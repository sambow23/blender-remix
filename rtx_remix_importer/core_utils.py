...
class TextureProcessor:
    ...
    async def convert_texture_async(
        self, 
        bl_image: bpy.types.Image, 
        output_path: str, 
        dds_format: str = 'BC7_UNORM_SRGB',
        ...
        def _convert():
            return self._convert_texture_sync(bl_image, output_path, dds_format, progress_callback)
        
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(get_thread_pool(), _convert)
    
    def _convert_texture_sync(
        self, 
        bl_image: bpy.types.Image, 
        output_path: str, 
        dds_format: str,
        ...
        """Synchronous texture conversion implementation."""
        try:
            if progress_callback:
                progress_callback(f"Converting {bl_image.name}...")
