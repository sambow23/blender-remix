...
class ImportCaptureFile(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            # Run import synchronously to avoid Blender 5.0+ crashes from dependency graph updates in background threads
...
class BatchImportSelectedCaptures(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            # Import all captures synchronously
            ...
            context.scene.render.use_simplify = viewport_state['use_simplify']
            context.scene.render.simplify_subdivision = viewport_state['simplify_subdivision']
